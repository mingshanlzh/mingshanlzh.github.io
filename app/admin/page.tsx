...
import {
  Settings, LogIn, Lock, BookOpen, Mic2, Trophy, Newspaper,
  MessageSquare, Eye, EyeOff, Upload, CheckCircle2, Tv2, X
} from "lucide-react";
...
const PAGE_VISIBILITY = [
  ...
  { id: "cv",           label: "CV",           visible: true },
  { id: "projects",     label: "Working Projects", visible: true },
  { id: "teaching",     label: "Teaching",     visible: true },
  { id: "supervision",  label: "Supervision",  visible: true },
  { id: "talks",        label: "Talks",        visible: true },
  { id: "research",     label: "Selected Research", visible: true },
  { id: "media",        label: "Media",        visible: true },
  { id: "awards",       label: "Awards & Grants", visible: true },
  { id: "services",     label: "Services",     visible: true },
  { id: "affiliations", label: "Affiliations", visible: true },
...
export default function AdminPage() {
  ...
  const [pages, setPages] = useState(PAGE_VISIBILITY);
  ...
  function handleLogin(e: React.FormEvent) {
    e.preventDefault();
    // In production: Supabase Auth with admin role check
    if (credentials.email === "shan.jiang@mq.edu.au" && credentials.password.length >= 6) {
      setLoggedIn(true);
    } else {
      setError("Invalid credentials. Only the site owner can access this panel.");
    }
  }
  ...
  return (
    <div style={{ maxWidth: "860px" }}>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setLoggedIn(false)} className="btn btn-outline text-xs">
          Sign out
        </button>
      </div>
      ...
      {activeTab === "news" && (
        <div>
          ...
          <p className="text-sm mb-4" style={{ color: "var(--text-muted)" }}>
            Enter a new item here and it will automatically appear on the News page and be routed to the relevant section (Publications, Talks, Awards, etc.).
          </p>
          {submitted && (
      ...
      {activeTab === "pages" && (
        <div>
          ...
          <p className="text-sm mb-4" style={{ color: "var(--text-muted)" }}>
            Toggle pages on/off. Hidden pages are removed from the navigation and return a 404.
          </p>
            ...
            {pages.map((page) => (
              <div key={page.id} className="card flex items-center justify-between gap-4">
                ...
                <button onClick={() => togglePage(page.id)}
                  ...
                  style={{
                    background: page.visible ? "#F0FFF4" : "#FFF5F5",
                    color: page.visible ? "#38A169" : "#E53E3E",
                    border: "none",
                    padding: "0.25rem 0.6rem",
                  }}>
                  {page.visible ? <Eye size={13} /> : <EyeOff size={13} />}
